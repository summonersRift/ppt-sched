...
class TorusSwitch(Switch):
    ...
    def print_stats(self, *args):
        ...
        for k,i in self.interfaces.iteritems():
            if (i.stats_total_sent_pkts()>0) or (i.stats_total_rcvd_pkts()>0):
                print ("[TorusSwitch-%d,%s] sent[bytes/pkts]:%d/%d, received[bytes/pkts]:%d/%d"
                    % (self.node_id, k,
                    i.stats_total_sent_bytes(), i.stats_total_sent_pkts(),
                    i.stats_total_rcvd_bytes(), i.stats_total_rcvd_pkts()  )  )
