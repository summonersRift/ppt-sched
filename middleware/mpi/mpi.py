...
def mpi_finalize_original(mpi_comm):
...
def mpi_finalize(mpi_comm):
    """It is required all mpi processes end with this function."""
    p = mpi_comm_rank(mpi_comm)
    str_me = " * mpi.py:mpi_finalize() --"

    if (p==0):            #process 0 requests termination evt
        delay = 0.000001
        print "--%s a job finished early." % (str_me)
        my_host = mpi_ext_host(mpi_comm)
        now     = my_host.get_now()
        params  = { "time" : now+delay,
                    "root" : my_host.node_id,
                    "cause": "normal"}
        my_host.reqService(delay, "job_finished_evt", params, "JobScheduler", 0)
    mpi_comm['mpiproc'].kill()


def mpi_finalize_old(mpi_comm):
    """It is required all mpi processes end with this function."""

    p = mpi_comm_rank(mpi_comm)
    str_me = " * mpi.py:mpi_finalize() --"
    print " * mpi.py-mpi_finalize() Job finished early. p:", p
 
    if (p==0):            #process 0 informs scheduler to terminate job
        my_host = mpi_ext_host(mpi_comm)
        sched_soft = my_host.hpcsim_dict['sched_node'].scheduler
        job_no = int(sched_soft.jobq.get_job_no_by_root_node(my_host.node_id))
        print "%s job_no:%d finished early." % (str_me, job_no)

        #if the application is terminated and we are trying to kill, its an error   
        state = sched_soft.jobq.get_status(job_no)
        if state == "finished-timeout":
            raise Exception("Job wasnt killed properly at timeout. this shouldnt be called")
        #else
        sched_soft.terminate_job(my_host.get_now(), "normal", job_no)
...
def mpi_recv(mpi_comm, from_rank=None, type=None):
    ...
    if ("mpi-recv" in host.hpcsim_dict["debug_options"]) and ('ttl' in recvitem):   #obaida 
        print("rank %d (comm=%d) mpi_recv: from_rank=%r, type=%r, ttl=%d: DONE" %
              (mpi_comm['rank'], mpi_comm['commid'], recvitem["from_rank"], recvitem["type"], recvitem['ttl'] ))
...
class MPIHost(Host):
    ...
    def create_mpi_proc(self, *args):
        ...
        if self.mpi_opt_set == False:
            self.mpi_opt_set = True
        ...
        # fix main_proc
        module_app = sys.modules[data["main_proc"]["module"]]
        all_functions = inspect.getmembers(module_app, inspect.isfunction)
        for item in all_functions:
            if item[0] == data["main_proc"]["app"]:
                data['main_proc'] = item[1]
                break


        # create the mpi main function
        proc_name = "%s_%d"% (data["main_proc"], data["rank"])
        #print (" * zzz mpi.py create_mpi_proc() proc_name@host: %s@%s"%
        #              (proc_name, self))
    ...
    def kill_mpi_proc(self, *args):   #zzz obaida 
        """A process killing service scheduled by job_timeout_evt in nodes.py."""

        str_me = "          * mpi.py::kill_mpi_proc() --"
        params = args[0] 
        main_proc = params[0]
        rank = params[1]

        #proc_name = "%s_%d"% (data["main_proc"], data['rank'])
        #print " * process names: ",self.getProcessNames()
        #print " * process list: ",self._procList

        proc =  self.getProcessByRegExp(main_proc, str(rank) )
        #proc =  self.killProcessByRegExp(main_proc)

        if proc is not None:
            #print " * kill_mpi_proc-",rank," killing:",proc
            self.killProcess(proc)
            #double check
            proc =  self.getProcessByRegExp(main_proc,str(rank) )
            if proc is not None:
                raise Exception ("THE PROCESS SHOULD BE DELETED ALREADY")
        else:
            #print "proc already deleted.proclist:", self._procList
            pass
